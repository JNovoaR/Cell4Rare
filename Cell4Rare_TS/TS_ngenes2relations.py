...
#imput command: python3 h5ad2genes.py singlecell_obs_data.tsv hpo_list_file  ngenes_tissue [ngenes_tissue] ... >oli_file


##-- IMPUTS --##

# singlecell_obs_data.tsv. FORMAT: tab-separated file; DESCRIPTION: table of qualitative features of each cell extracted from the h5ad with extract_h5ad_obs.py
...
# ngenes_tissue. FORMAT: tab-separated file; DESCRIPTION: ngenes file [at least one, but can be more, one after the other as sucessive arguments] from h5ad2ngenes.py

##-- OUTPUTS --##

# oli_file. FORMAT: tab-separated file; DESCRIPTION: File containing the final oli results [Over the LIne, in reference to the regression line analisis). It containts the following columns:

#1- hpo id
#2- hpo name
#3- number of genes related to the hpo
#4- agrupation level
#5- group name
#6- nº cells belonging to group
#7- nº of backgroun cells
#8- ks statistic
#9- ks pvalue
#10- size effect
#11- significant (<0.01)
...
for included_tissue in l_tissues:
	with open(f_cell_annotation, "r") as f:
		for line in f:
			...
			cl_name = l_line[7].lower()
			manual_name = l_line[8].lower()
			group = l_line[10].lower()
			if tissue == included_tissue:
				...
				if f"{tissue}/{group}/{cl_name}" not in d_celltypengroupntissue2cell.keys():
					d_celltypengroupntissue2cell[f"{tissue}/{group}/{cl_name}"] = [n_cell]
				else:
					d_celltypengroupntissue2cell[f"{tissue}/{group}/{cl_name}"].append(n_cell)
				n_cell += 1
...
for id_n_name in l_hpos:
	...
	### RELEVANT TO ARODRIGUEZ ###
	list_of_dictionaries = [d_tissue2cell, d_groupntissue2cell, d_celltypengroupntissue2cell]
	list_of_agrupation_levels = ["tissue", "group", "cell-type"]
	### ---------------------- ###
